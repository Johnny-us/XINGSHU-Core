...
v0.4 是 Runnable Core / Validator CLI Candidate（可运行核心 / 验证器命令行候选）。Runnable（可运行）不等于 Active Governance（治理已激活），也不构成 Adoption、Release 或 Personal Instance 自动接入。

## Added（新增）

- Python 3.11+ 的 `xingshu-core` Editable Package（可编辑安装包）；
...
- 读取现有 `schemas/v0.3/` 的 Schema Registry，不复制第二套 Schema；
- 三个纯合成 Examples 和 subprocess Runtime Tests。

## Preserved（保持）

- v0.3 Schema 没有重新定义；
- v0.3 Conformance 测试改为调用 Runtime Library，原 Fixtures 和预期语义保持；
- v0.2 Conformance 与 v0.1↔v0.2 Compatibility 保持；
...
第一版支持 Clone / Checkout 后执行 `pip install -e .`。独立 Wheel Packaging（Wheel 打包）和非仓库 Schema 分发尚未实现。
