...
def _doctor_report() -> tuple[dict[str, Any], int]:
    ...
    add("python_version", sys.version_info >= (3, 11), ".".join(str(part) for part in sys.version_info[:3]))
    ...
    add("repository_root", root.is_dir(), "available" if root.is_dir() else "unavailable")

    try:
        registry = SchemaRegistry()
        discovered = registry.discover()
        add("schema_root", True, "canonical")
    ...
    try:
        ...
        manifest_ok = "manifest_version:" in text and "capabilities:" in text
        add("manifest", manifest_ok, "present" if manifest_ok else "invalid")
    except OSError:
...
def build_parser() -> SafeArgumentParser:
    parser = SafeArgumentParser(
        prog="xingshu", description="Read-only XINGSHU JSON validator"
    )
